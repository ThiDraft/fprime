        ...
        for interface_tag in interface:
            if interface_tag.tag == "comment":
                ...
                for arg in interface_tag:
                    if arg.tag != "arg":
                        PRINT.info(
                            "%s: Invalid tag %s in interface args definition", xml_file, arg.tag)
                        sys.exit(-1)
                    n = arg.attrib["name"]
                    t = arg.attrib["type"]
                    ...
                    for arg_tag in arg:
                        ...
                        if arg_tag.tag == "comment":
                            ...
                            PRINT.info(
                                "%s: Invalid argument tag %s in port %s argument %s", xml_file, arg_tag.tag, interface_tag.tag, n)
                            sys.exit(-1)
                ...
                for enum_tag in interface_tag:
                    ...
                    if enum_tag.tag == "enum" and t == "ENUM":
                        ...
                        PRINT.info(
                            "%s: Invalid port return value tag %s", xml_file, enum_tag.tag)
                        sys.exit(-1)
